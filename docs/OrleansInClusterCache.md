...
> **Note:** The `Action<InClusterCacheOptions>? setupAction` parameter is optional and can be used to configure default options.
...
To cache a value using in-cluster cache grains, start by creating a marker interface that inherits one or more of the following interfaces, depending on the caching strategy you want to use:
...
In-cluster cache grains expose several methods for interaction:
...
Here, `grainFactory` is an instance of Microsoft Orleans’ `IGrainFactory`.
