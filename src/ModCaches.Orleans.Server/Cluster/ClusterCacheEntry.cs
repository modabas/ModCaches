...
public record ClusterCacheEntry<TValue>(
  TValue Value,
  ClusterCacheEntryOptions Options)
  where TValue : notnull;

public static class ClusterCacheEntry
{
  /// <summary>
  /// Creates a new instance of the <see cref="ClusterCacheEntry{TValue}"/> class using the specified value and cache entry options.
  /// </summary>
  ...
  /// <returns>A <see cref="ClusterCacheEntry{TValue}"/> instance containing the specified value and cache entry options.</returns>
  public static ClusterCacheEntry<TValue> Create<TValue>(
    TValue value,
    ClusterCacheEntryOptions options)
    where TValue : notnull
  {
    return new ClusterCacheEntry<TValue>(value, options);
  }
  ...
  /// <param name="value">The value to be stored in the cache entry.</param>
  /// <param name="options">The options used to configure the cache entry.</param>
  /// <returns>A successful result containing a new <see cref="ClusterCacheEntry{TValue}"/> instance initialized with the specified
  /// value and options.</returns>
  public static Result<ClusterCacheEntry<TValue>> CreateResult<TValue>(
    TValue value,
    ClusterCacheEntryOptions options)
    where TValue : notnull
  {
