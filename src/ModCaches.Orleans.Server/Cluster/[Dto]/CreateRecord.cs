...
{
  /// <summary>
  /// Creates a new instance of the <see cref="CreateRecord{TValue}"/> class using the specified value and cache entry options.
  /// </summary>
  ...
  /// <returns>A <see cref="CreateRecord{TValue}"/> instance containing the specified value and cache entry options.</returns>
  public static CreateRecord<TValue> From<TValue>(
    TValue value,
    CacheGrainEntryOptions options)
    where TValue : notnull
  {
    return new CreateRecord<TValue>(value, options);
  }
}
