...
public class CoHostedOrleansVolatileCacheTests
{
  ...
  public async Task Get_ReturnsBytesAsync()
  {
    ...
    grainFactory.GetGrain<IVolatileDistributedCacheGrain>(Arg.Any<string>()).Returns(grain);

    var cache = new CoHostedOrleansVolatileCache(grainFactory);
    ...
    grainFactory.Received(1).GetGrain<IVolatileDistributedCacheGrain>(Key);
  }
